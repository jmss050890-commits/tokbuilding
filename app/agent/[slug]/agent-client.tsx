  ...
  const speak = (text: string) => {
    ...
    if (!selectedVoice && agent?.voiceGender) {
      if (agent.voiceGender === "female") {
        const preferredFemaleVoices = ["zira", "samantha", "victoria", "moira", "karen", "fiona"];
        selectedVoice = voices.find(voice =>
          preferredFemaleVoices.some(pattern => voice.name.toLowerCase().includes(pattern))
        );
      } else {
        const preferredMaleVoices = ["aaron", "daniel", "david", "alex", "tom"];
        selectedVoice = voices.find(voice =>
          preferredMaleVoices.some(pattern => voice.name.toLowerCase().includes(pattern))
        );
      }
    }
    ...
    if (!selectedVoice && agent?.voiceGender) {
      if (agent.voiceGender === "female") {
        ...
        const femaleVoicePatterns = ["zira", "samantha", "victoria", "moira", "karen", "fiona", "anna", "emma", "bella"];
        selectedVoice = voices.find(v => {
          const isEnUS = v.lang?.includes("en-US") || v.lang?.includes("en-us");
